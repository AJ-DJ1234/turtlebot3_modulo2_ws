...
class HazmatInterface(Node):
    ...
    def update_map_with_markers(self):
        # Primero, limpiar el canvas para redibujar el mapa y los marcadores
...
def create_interface():
    ...
    markers_list.pack(side="left", fill="y", padx=5, expand=True)
    ...
    status_label.grid(row=2, column=0, columnspan=2, sticky="ew", padx=10, pady=5)
...
def main(args=None):
    ...
    # Iniciar rclpy.spin en un hilo separado
    threading.Thread(target=rclpy.spin, args=(node,), daemon=True).start()
    create_interface()
    # Asegurarse de que ROS2 se cierre al finalizar
    if rclpy.ok():
        rclpy.shutdown()
